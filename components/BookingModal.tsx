...
const BookingModal: React.FC<Props> = ({ open, onClose, instructorName, availableTimes }) => {
  ...
  const timesForDay = availableTimes.find((a) => a.day === selectedDay)?.times || [];
  ...
  return (
    <div className="fixed inset-0 bg-black/30 z-40 flex items-center justify-center transition">
      <div className="bg-white rounded-lg shadow-lg max-w-sm w-full p-6 relative animate-fade-in">
        ...
        <h3 className="text-xl font-semibold mb-4">Book Lesson with {instructorName}</h3>
        <div className="mb-3">
